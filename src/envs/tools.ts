...
export const getToolsConfig = () => {
  return createEnv({
    runtimeEnv: {
      ...
      DISABLE_CLOUD_SANDBOX: process.env.DISABLE_CLOUD_SANDBOX === '1',
      SANDBOX_BASE_URL: process.env.SANDBOX_BASE_URL,
      SANDBOX_PROVIDER: process.env.SANDBOX_PROVIDER,
      SANDBOX_WORKSPACE: process.env.SANDBOX_WORKSPACE,
    ...
    server: {
      ...
      DISABLE_CLOUD_SANDBOX: z.boolean().optional(),
      SANDBOX_BASE_URL: z.string().url().optional(),
      SANDBOX_PROVIDER: z.enum(['agent-infra', 'market']).optional(),
      SANDBOX_WORKSPACE: z.string().optional(),
